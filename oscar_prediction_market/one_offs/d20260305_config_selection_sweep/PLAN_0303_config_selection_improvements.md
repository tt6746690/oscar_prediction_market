...
14. [Not Recommended](#14-not-recommended)
15. [Systematic Model Selection (d20260305)](#15-systematic-model-selection-d20260305)
16. [Targeted Config Sweep Design (d20260305)](#16-targeted-config-sweep-design-d20260305)
...
- **Spearman ρ (EV vs actual):** Within a single model, how well the *rank
  ordering* of its 588 configs by EV predicts the rank ordering by actual PnL
  (averaged over 2024+2025). ρ=1.0 means the config that EV says is #1 is
  also #1 by actual, #2 is also #2, etc. All models have ρ > 0.90, meaning EV
  is an excellent *within-model* config selector.

- **Inflation:** `mean(EV) / mean(actual PnL)` across all 588 configs for that
  model.  Measures how much the model's self-assessed expected return overstates
  reality. 1.0x = EV perfectly calibrated. 3.39x (clogit) = EV is 3.4× what
  you actually get on average. Caused by overconfident probabilities inflating
  both Kelly sizing and EV weighting (the two channels in Section 3).

- **EV-best actual capture:** If you trust EV and pick the #1-by-EV config
  *within this model*, what fraction of that model's *best possible* actual PnL
  do you get?  Formally:
  `actual_pnl(config ranked #1 by EV) / actual_pnl(config ranked #1 by actual)`,
  both measured within the same model on combined 2024+2025 per-entry averages.
  92% for cal_sgbt means EV-based selection loses only 8% vs perfect hindsight
  within cal_sgbt.

#### Both Inflation AND Capture Matter

Note that **inflation and capture are not redundant** — they measure different
things and a model can score well on one but poorly on the other:

- avg_ensemble has the **lowest inflation** (1.27x) but the **lowest capture**
  (75%). Low inflation means the EV dollar amounts are close to reality, but
  the EV ranking within avg_ensemble still picks the wrong config: the
  EV-optimal config (`kf=0.5, edge=0.04`, very aggressive) is quite different
  from the actual-optimal config (`kf=0.05, edge=0.15`, conservative).

- cal_sgbt has the **highest capture** (92%) but moderate inflation (1.74x).
  Its EV dollar amounts overstate reality by ~74%, but the *relative ordering*
  of those inflated values tracks actual PnL extremely well.

The ideal model would have **both low inflation AND high capture**. In practice,
we may need to accept a tradeoff: use a low-inflation model (avg_ensemble) to
get trustworthy EV magnitudes for Pareto selection, while understanding that
the top-by-EV config may not be the very best actual config.

### Cross-Model vs Within-Model Correlation Gap

The within-model Spearman correlations above (all >0.90) show EV is an
excellent selector when the model is fixed. But when we pool all 3,528 configs
across all 6 models, the correlation drops:

| Scope | Spearman ρ (EV vs actual) | N configs |
|---|---|---|
| **Cross-model (all pooled)** | **0.882** | 3,528 |
| Within cal_sgbt | 0.976 | 588 |
| Within clogit | 0.959 | 588 |
| Within gbt | 0.950 | 588 |
| Within clogit_cal_sgbt_ens | 0.948 | 588 |
| Within avg_ensemble | 0.935 | 588 |
| Within lr | 0.907 | 588 |

The ~0.06 gap (0.882 cross-model vs ~0.95 average within-model) is entirely
due to clogit configs clustering at the top of EV rankings despite not being
at the top of actual rankings. This gap quantifies exactly why "fix model
first, then use EV for config selection" (Section 9) works better than using
EV to jointly select model + config.
...
**Note:** All metrics are computed on **combined 2024+2025 per-entry averages.**
"EV-best actual" = avg_actual_pnl of the config with the highest
avg_ev_pnl_blend within that model. "Capture" = EV-best's actual / model's
hindsight-best actual. "Overall rank" = position among all 3,528 configs
sorted by avg_actual_pnl descending.
...
### Why Fixing the Model Makes EV Reliable for Config Selection

The core insight is a **within-model vs cross-model** distinction:

- **Cross-model:** Different models have different calibration profiles (clogit
  is 3.4× inflated, avg_ensemble is 1.3× inflated). Comparing their EVs is
  apples-to-oranges — clogit "wins" by inflation, not by being better. E[PnL]
  is not a fair comparison because the probability biases differ.

- **Within model:** The calibration profile is *constant* across all 588
  configs for that model. So EV's relative ranking of configs reflects real
  differences in trading parameters (edge threshold, kelly fraction, etc.),
  not probability biases. That's why within-model Spearman >0.90 for all
  models — EV is excellent at ordering configs when the probability profile
  is held fixed.

**Analogy:** It's like comparing test scores across schools with different
grading curves — straight comparison is misleading (cross-model). But within
a school, the curve is the same for everyone, so relative rankings are
meaningful (within-model). Fixing the model is fixing the grading curve.
...
Rationale (empirical + structural):

1. **Empirical (2024+2025):** Within avg_ensemble, actual PnL peaks at higher
   edge thresholds despite EV preferring lower ones. The data shows edge=0.15
   configs earn more actual PnL than edge=0.04 configs.

2. **Structural (quality over quantity):** Lower edge thresholds include bets
   with near-zero edge. For binary outcomes, these bets have ~50/50 win rate
   relative to costs — they deploy capital but don't reliably generate profit.
   Higher thresholds concentrate capital on high-confidence bets where the
   model has meaningful edge over the market.

3. **Kelly theory:** Kelly sizing already sizes positions proportionally to
   edge. But Kelly assumes the probabilities are correct. When model probs are
   overconfident (they always are — even avg_ensemble is 1.27x inflated),
   Kelly oversizes marginal bets. An edge threshold acts as a pre-filter that
   removes the least trustworthy bets before Kelly sees them.

The structural argument applies regardless of how many backtest years we have
— it's a consequence of model miscalibration interacting with Kelly sizing.
...
## 15. Systematic Model Selection (d20260305)

The conflicting evidence from the analysis above makes model selection
non-trivial. Here we define a systematic scorecard approach.

### Model Scorecard Framework

For each model, compute and compare these signals:

| Signal | Meaning | What it favors |
|---|---|---|
| Both-year profitable % | Fraction of 588 configs profitable in both 2024+2025 | Robustness/consistency |
| Mean combined actual PnL | Average across all configs | Overall performance |
| Best combined actual PnL | Ceiling — how good can this model get? | Upside potential |
| EV inflation (lower = better) | How much EV overstates actual | Trustworthy EV for Pareto |
| Within-model capture | How well EV picks the right config | EV as config selector |
| Within-model Spearman ρ (EV↔actual) | Rank preservation of EV within model | EV reliability |
| Cross-year Spearman ρ (actual_24↔actual_25) | Do config rankings transfer across years? | Temporal stability |
| Mean 2024 actual / Mean 2025 actual | Year balance | Not over-reliant on one year |

### Current Scorecard (from d20260225 results)

| Signal | avg_ens | cal_sgbt | clogit | clog_sgbt | gbt | lr |
|---|---|---|---|---|---|---|
| Both % | 92.9% | 66.7% | **100%** | 92.9% | 83.3% | **100%** |
| Mean combined | $11,711 | **$12,111** | $8,396 | $11,192 | $11,687 | $8,833 |
| Best combined | **$30,043** | $27,713 | $19,533 | $26,974 | $30,004 | $21,382 |
| Inflation | **1.27x** | 1.74x | 3.39x | 1.57x | 1.83x | 1.87x |
| Capture | 75% | **92%** | 85% | 79% | 77% | 75% |
| Spearman ρ (EV↔act) | 0.935 | **0.976** | 0.959 | 0.948 | 0.950 | 0.907 |
| Cross-year ρ | 0.790 | 0.586 | 0.893 | 0.734 | 0.615 | **0.913** |
| Mean 2024 | $2,230 | $413 | **$3,290** | $2,355 | $776 | $2,267 |
| Mean 2025 | $9,481 | **$11,698** | $5,106 | $8,838 | $10,910 | $6,566 |

**Observations:**
- No model dominates all dimensions. The choice depends on which properties
  we prioritize.
- **clogit** wins on robustness (100% profitable, high cross-year ρ) and 2024
  performance, but its 3.39x EV inflation makes EV-based config selection
  pick the wrong configs.
- **cal_sgbt** wins on capture (92%) and within-model Spearman (0.976) — if we
  fix the model, EV is most reliable here. But it's inconsistent across years
  (66.7% both-profitable, 0.586 cross-year ρ, $413 mean 2024).
- **avg_ensemble** is the balanced choice: lowest inflation (1.27x), highest
  ceiling ($30k), good-but-not-best on most dimensions. Its weakness is low
  capture (75%) — EV doesn't pick the best config within it.
- **gbt** is surprisingly strong on combined PnL but has 83.3% robustness.
- **lr** has excellent temporal stability (0.913 cross-year ρ) but modest PnL.

### Planned Analysis (d20260305 one-off)

Re-run all backtests with refactored upstream code (post-renormalization
removal) and recompute the model scorecard. The upstream changes are:
- Removed probability renormalization for lr and gbt (no-op for clogit,
  cal_sgbt, ensembles)
- This may change relative model performance for gbt and lr

The d20260305 one-off will:
1. Run fresh backtests (all 6 models, targeted config grid)
2. Compute the full model scorecard table above
3. Add cross-model vs within-model Spearman analysis
4. Add per-model Pareto frontiers
5. Add bootstrap rank stability analysis
6. Recommend a model based on the evidence

---

## 16. Targeted Config Sweep Design (d20260305)

Rather than the full 588-config grid, we use domain knowledge from d20260225
to focus on a refined sweep:

### Fixed Parameters (from d20260225 findings)

| Parameter | Value | Justification |
|---|---|---|
| fee_type | taker | Conservative — realistic worst-case fees |
| kelly_mode | multi_outcome | Consistently outperforms independent across all models and years |
| allowed_directions | all | Consistently outperforms yes-only and no-only |
...
**Note on kelly_fraction with multi_outcome:** In multi_outcome mode,
kelly_fraction only sets the optimizer's initial guess (not a direct scaling
factor). The optimizer is free to find a different optimum. In practice,
different starting points can lead to different local optima (SLSQP is a local
solver), so we include a few values as a robustness check. If results are
stable across kelly_fraction, we can treat it as a non-dial.

**Total: 9 × 3 = 27 configs per model, × 6 models = 162 total.** This is
much faster to run than the full 588 × 6 = 3,528 grid while providing finer
edge threshold resolution where it matters.
